/*
  atom/tello: https://github.com/atom/tello/tree/master
  MIT

  This file has been forked from the tello repository.
  Decaffeinated, and received small modifications to ensure it works as expected here.
*/
...
class Digester {
  ...
  */
  ...
  sectionFromCommentEntity(commentEntity) {
    const doc = this.opts.pulsardoc(commentEntity.doc);
  ...
  docFromDocString(docString, options) {
    let classDoc;
    if (docString != null) { classDoc = this.opts.pulsardoc(docString, options); }
...
module.exports = {
  digest(metadata, opts) {
    return new Digester(opts).digest(metadata);
  }
};
